...
pub(crate) fn active_media_task_prompt_section(
    ...
    let mut lines = vec![
        ...
        "- `voice.speech` controls are model-specific. If this active media task is a video / 口播视频 / 数字人 continuation, the video workflow must remain owned by `video-director`; do not start from `cosyvoice-ssml`. For CosyVoice models such as cosyvoice-v3.5-plus, activate `cosyvoice-ssml` only when `video-director` has already confirmed a digital-human / VideoRetalk / asset-library talking-head task and this is the approved character speech-track TTS substep. In that narrow flow, activation only updates instructions and will not return SSML; use ordered `segments` with complete `<speak rate=\"...\" pitch=\"...\" volume=\"...\">...</speak>` SSML inputs plus per-segment prompts. For ordinary audio or non-digital-human tasks, keep CosyVoice payloads conservative and do not activate `cosyvoice-ssml`. For MiniMax models such as speech-2.8-turbo, activate `tts-director` for expressive work and use `emotion`, speed/pitch, MiniMax pause markers like <#0.6#>, and ordered `segments` when needed.".to_string(),
    ];
