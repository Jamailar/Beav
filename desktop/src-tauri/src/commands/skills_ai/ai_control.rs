...
pub(super) fn ai_roles_list_value() -> Result<Value, String> {
    Ok(json!([
        {
            "roleId": "planner",
...
pub(super) fn ai_detect_protocol_value(payload: &Value) -> Result<Value, String> {
...
pub(super) fn ai_test_connection_value(payload: &Value) -> Result<Value, String> {
    let base_url = payload_string(payload, "baseURL").unwrap_or_default();
