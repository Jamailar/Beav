...
pub(crate) struct SubjectMediaInput {
...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct SubjectVoiceInput {
    pub(crate) relative_path: Option<String>,
    pub(crate) data_url: Option<String>,
    pub(crate) name: Option<String>,
    pub(crate) script_text: Option<String>,
    pub(crate) voice: Option<Value>,
}

#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct SubjectMutationInput {
    pub(crate) id: Option<String>,
    pub(crate) name: String,
    pub(crate) category_id: Option<String>,
    pub(crate) description: Option<String>,
    pub(crate) tags: Option<Vec<String>>,
    pub(crate) attributes: Option<Vec<SubjectAttribute>>,
    pub(crate) images: Option<Vec<SubjectMediaInput>>,
    pub(crate) voice: Option<SubjectVoiceInput>,
    pub(crate) video: Option<SubjectMediaInput>,
    pub(crate) brand_id: Option<String>,
    pub(crate) skus: Option<Vec<SubjectSku>>,
}

#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct SubjectCategoryMutationInput {
    pub(crate) id: Option<String>,
    pub(crate) name: String,
}
...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct YoutubeSavePayload {
...
pub(crate) struct FileNode {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub(crate) richpost_preview_file: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub(crate) richpost_preview_file_url: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub(crate) richpost_preview_updated_at: Option<i64>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub(crate) richpost_preview_page_file: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub(crate) richpost_preview_page_file_url: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub(crate) richpost_preview_page_updated_at: Option<i64>,
}
...
pub(crate) fn ensure_runtime_warm_entry(
    state: &State<'_, AppState>,
    mode: &str,
...
pub(crate) fn normalize_string(value: Option<&Value>) -> Option<String> {
    value
...
pub(crate) fn payload_value_as_string(payload: &Value) -> Option<String> {
...
pub(crate) fn collect_text_files_recursive(root: &Path, max_depth: usize, out: &mut Vec<PathBuf>) {
    if max_depth == 0 {
        return;
    }
...
pub(crate) fn build_excerpt_around(content: &str, max_chars: usize) -> String {
    let normalized = content.replace('\0', "").replace("\r\n", "\n");
    truncate_chars(normalized.trim(), max_chars)
}

pub(crate) fn load_advisor_existing_context(store: &AppStore, advisor_id: &str) -> String {
...
pub(crate) fn render_named_corpus(
    label: &str,
    items: &[(String, String)],
    empty_text: &str,
) -> String {
...
pub(crate) fn collect_advisor_knowledge_evidence(
    state: &State<'_, AppState>,
    advisor_id: &str,
...
pub(crate) fn collect_related_manuscript_evidence(
    state: &State<'_, AppState>,
...
pub(crate) fn load_skill_bundle_sections(
    state: &State<'_, AppState>,
    skill_name: &str,
...
pub(crate) fn default_work_schedule() -> WorkScheduleRecord {
    WorkScheduleRecord {
        mode: "none".to_string(),
...
pub(crate) fn default_work_refs() -> WorkRefsRecord {
    WorkRefsRecord {
        project_ids: Vec::new(),
        session_ids: Vec::new(),
        task_ids: Vec::new(),
...
pub(crate) fn collect_sample_files(root: &Path, limit: usize) -> Result<Vec<String>, String> {
    let mut files = Vec::new();
    if !root.exists() {
        return Ok(files);
    }
...
pub(crate) fn count_files_in_dir(root: &Path) -> Result<i64, String> {
    if !root.exists() {
        return Ok(0);
    }
    let mut count = 0_i64;
...
pub(crate) fn guess_mime_and_kind(path: &Path) -> (String, String, bool) {
    let ext = path
        .extension()
        .and_then(|value| value.to_str())
        .unwrap_or("")
...
pub(crate) fn video_thumbnail_key(path: &Path) -> String {
...
#[cfg(target_os = "macos")]
pub(crate) fn run_osascript_json(script: &str) -> Result<Value, String> {
...
pub(crate) fn run_powershell_json(script: &str) -> Result<Value, String> {
...
pub(crate) fn decode_command_json_stdout(bytes: &[u8]) -> String {
    if bytes.is_empty() {
        return String::new();
    }
...
#[cfg(any(target_os = "windows", test))]
pub(crate) fn decode_utf16_stdout(bytes: &[u8], little_endian: bool) -> String {
    let units = bytes
        .chunks_exact(2)
        .map(|chunk| {
            if little_endian {
...
pub(crate) fn json_value_to_path_list(value: &Value) -> Vec<PathBuf> {
    match value {
        Value::String(path) => {
...
pub(crate) fn pick_files_native(
    prompt: &str,
    folders_only: bool,
    multiple: bool,
...
pub(crate) fn file_content_hash(path: &Path) -> Result<String, String> {
...
pub(crate) fn write_debug_line_to_store(store: &Arc<Mutex<AppStore>>, line: &str) {
...
pub(crate) fn register_global_debug_store(store: Arc<Mutex<AppStore>>) {
    let _ = GLOBAL_DEBUG_STORE.set(store);
}

pub(crate) fn register_global_app_handle(app: AppHandle) {
...
pub(crate) fn build_chat_error_payload(error: &str, session_id: Option<String>) -> Value {
...
pub(crate) fn redclaw_state_value(state: &RedclawStateRecord) -> Value {
...
pub(crate) fn knowledge_version(store: &AppStore) -> String {
    format!(
        "{}:{}:{}:{}",
...
pub(crate) fn knowledge_source_texts(store: &AppStore) -> Vec<(String, String, Value)> {
    let mut items = Vec::new();
...
pub(crate) fn wander_item_from_note(note: &KnowledgeNoteRecord) -> Value {
...
pub(crate) fn wander_item_from_youtube(video: &YoutubeVideoRecord) -> Value {
    json!({
        "id": video.id,
        "type": "video",
        "title": video.title,
...
pub(crate) fn wander_item_from_doc(source: &DocumentKnowledgeSourceRecord) -> Value {
    json!({
        "id": source.id,
        "type": "note",
        "title": source.name,
...
pub(crate) fn build_wander_material_ref(
    kind: &str,
    source_type: &str,
    storage_root: &str,
...
pub(crate) fn derive_workspace_material_path(
    storage_root: &str,
    folder_path: Option<&str>,
    fallback_leaf: &str,
) -> String {
...
pub(crate) fn build_wander_items_text(items: &[Value]) -> String {
    items
        .iter()
        .enumerate()
        .map(|(index, item)| {
...
pub(crate) fn resolve_wander_model_config(_settings: &Value) -> Value {
    json!({
        "runtimeMode": "wander"
    })
}

pub(crate) fn generate_wander_response(
    app: &AppHandle,
    state: &State<'_, AppState>,
    session_id: &str,
    config: &Value,
...
pub(crate) fn reject_parent_directory_traversal(raw_path: &str) -> Result<(), String> {
...
pub(crate) fn resolve_editor_tool_file_path(
    state: &State<'_, AppState>,
    session_id: Option<&str>,
    arguments: &Value,
) -> Result<String, String> {
...
pub(crate) fn editor_tool_payload(file_path: String, arguments: &Value, keys: &[&str]) -> Value {
...
pub(crate) fn model_config_value_from_resolved(config: &ResolvedChatConfig) -> Value {
    let mut value = json!({
...
pub(crate) fn openai_reasoning_effort_default(config: &ResolvedChatConfig) -> Option<&'static str> {
...
pub(crate) fn apply_openai_reasoning_effort(config: &ResolvedChatConfig, body: &mut Value) {
...
#[derive(Default)]
pub(crate) struct StreamingToolDelta {
    pub(crate) id: String,
    pub(crate) name: String,
...
#[derive(Default)]
pub(crate) struct StreamingChatCompletion {
    pub(crate) content: String,
    pub(crate) tool_calls: Vec<InteractiveToolCall>,
...
pub(crate) fn interactive_attachment_string_field(attachment: &Value, key: &str) -> Option<String> {
    attachment
        .get(key)
        .and_then(Value::as_str)
        .map(str::trim)
...
pub(crate) fn interactive_attachment_delivery_mode(attachment: &Value) -> String {
    interactive_attachment_string_field(attachment, "deliveryMode")
        .unwrap_or_default()
        .to_ascii_lowercase()
}

pub(crate) fn interactive_attachment_kind(attachment: &Value) -> String {
    interactive_attachment_string_field(attachment, "kind")
        .unwrap_or_else(|| "binary".to_string())
        .to_ascii_lowercase()
}

pub(crate) fn interactive_attachment_inline_data_url(
    attachment: &Value,
...
pub(crate) fn interactive_base64_payload_size(base64_data: &str) -> u64 {
...
pub(crate) fn interactive_transport_supports_direct_attachment(
    protocol: &str,
    attachment_kind: &str,
) -> bool {
    match protocol {
...
pub(crate) fn interactive_model_supports_direct_attachment(
    protocol: &str,
    model_name: &str,
    attachment_kind: &str,
    mime_type: &str,
...
pub(crate) fn interactive_direct_attachment_max_bytes(
    protocol: &str,
    attachment_kind: &str,
) -> u64 {
...
pub(crate) fn interactive_attachment_direct_input_payload(
    attachment: &Value,
    protocol: &str,
    model_name: &str,
...
pub(crate) fn interactive_attachment_fallback_note(
    attachment: &Value,
    protocol: &str,
    model_name: &str,
) -> Option<String> {
...
pub(crate) fn interactive_attachment_tool_read_note(
    attachment: &Value,
...
pub(crate) fn interactive_attachment_tool_reference_note(attachment: &Value) -> Option<String> {
    let name = interactive_attachment_string_field(attachment, "name")
        .unwrap_or_else(|| "attachment".to_string());
    let kind = interactive_attachment_kind(attachment);
...
pub(crate) fn interactive_history_attachment_note(
    attachment: &Value,
...
pub(crate) fn interactive_attachment_items(attachment: Option<&Value>) -> Vec<&Value> {
    match attachment {
...
pub(crate) fn compose_user_message_text(base_message: &str, note: Option<&str>) -> String {
...
pub(crate) fn build_interactive_user_turn_messages(
    message: &str,
...
pub(crate) fn canonical_text_message(role: &str, content: String) -> Value {
    json!({
        "role": role,
        "content": content
    })
...
pub(crate) fn canonical_assistant_message(
    content: String,
    tool_calls: &[InteractiveToolCall],
) -> Value {
    json!({
        "role": "assistant",
        "content": content,
...
pub(crate) fn canonical_tool_result_message(
    call_id: &str,
    tool_name: &str,
    content: String,
    success: bool,
...
pub(crate) fn validate_runtime_tool_message_sequence(messages: &[Value]) -> Result<(), String> {
...
pub(crate) fn canonical_messages_to_openai_messages(messages: &[Value]) -> Vec<Value> {
    messages
        .iter()
        .filter_map(|message| {
...
pub(crate) fn input_attachments_for_message(message: &Value) -> Vec<Value> {
...
pub(crate) fn canonical_messages_to_anthropic_messages(messages: &[Value]) -> Vec<Value> {
    messages
        .iter()
        .filter_map(|message| {
            let role = message.get("role").and_then(Value::as_str).unwrap_or("");
...
pub(crate) fn canonical_messages_to_gemini_contents(messages: &[Value]) -> Vec<Value> {
    messages
        .iter()
        .filter_map(|message| {
            let role = message.get("role").and_then(Value::as_str).unwrap_or("");
...
pub(crate) fn save_runtime_session_bundle(
    state: &State<'_, AppState>,
    session_id: Option<&str>,
    protocol: &str,
    runtime_mode: &str,
...
pub(crate) fn finalize_interactive_runtime_state(
    state: &State<'_, AppState>,
    session_id: Option<&str>,
    content: &str,
    error: Option<&str>,
...
pub(crate) fn ensure_interactive_runtime_not_cancelled(
    state: &State<'_, AppState>,
    session_id: Option<&str>,
) -> Result<(), String> {
    if session_id
...
pub(crate) fn append_prompt_and_canonical_message(
...
pub(crate) fn append_runtime_tool_media_attachments(
    prompt_messages: &mut Vec<Value>,
    canonical_messages: &mut Vec<Value>,
    result: &Value,
    protocol: &str,
...
pub(crate) fn build_interactive_tool_outcome_digest(
    tool_name: &str,
    arguments: &Value,
    success: bool,
    content: &str,
...
pub(crate) fn tool_action_name(arguments: &Value) -> Option<String> {
...
pub(crate) fn ensure_structured_tool_success(
    tool_name: &str,
    action: Option<&str>,
    result: Value,
...
pub(crate) fn structured_tool_success_payload(
    tool_name: &str,
    action: Option<&str>,
    data: Value,
    tool_plan_fingerprint: Option<&str>,
...
pub(crate) fn ensure_structured_tool_error(
    tool_name: &str,
    action: Option<&str>,
    error: &str,
    tool_plan_fingerprint: Option<&str>,
...
pub(crate) fn insert_tool_plan_meta(
...
pub(crate) fn interactive_tool_call_description(tool_name: &str, arguments: &Value) -> String {
...
pub(crate) struct InteractiveSkillActivation {
    pub(crate) name: String,
    pub(crate) description: Option<String>,
...
pub(crate) fn tool_result_data<'a>(result: &'a Value) -> &'a Value {
    result.get("data").unwrap_or(result)
}

pub(crate) fn structured_tool_payload_from_text(text: &str) -> Option<Value> {
    serde_json::from_str::<Value>(text)
        .ok()
        .filter(|value| value.is_object())
}

pub(crate) fn structured_tool_error_code(text: &str) -> Option<String> {
...
pub(crate) struct InteractiveExecutionContract {
    pub(crate) require_source_read: bool,
...
#[derive(Debug, Clone, Default)]
pub(crate) struct InteractiveExecutionProgress {
    pub(crate) source_read_completed: bool,
...
pub(crate) fn redbox_fs_profile_read_completed(arguments: &Value) -> bool {
    let action = tool_action_name(arguments)
        .unwrap_or_default()
        .to_ascii_lowercase();
...
pub(crate) fn manuscript_save_result_path(result: &Value) -> Option<&str> {
    let data = tool_result_data(result);
...
pub(crate) fn manuscript_save_result_content(result: &Value) -> Option<&str> {
    let data = tool_result_data(result);
    let string_paths = [
        &["content"][..],
        &["body"][..],
...
pub(crate) fn manuscript_save_result_has_content(result: &Value) -> bool {
    let data = tool_result_data(result);
...
pub(crate) fn normalized_app_cli_action_key(arguments: &Value) -> String {
    payload_string(arguments, "action")
        .unwrap_or_default()
        .chars()
...
pub(crate) fn metadata_requires_voice_speech(metadata: &Value) -> bool {
...
pub(crate) fn clear_interactive_execution_contract_metadata(
...
pub(crate) fn clear_completed_interactive_execution_contract(
    state: &State<'_, AppState>,
    session_id: Option<&str>,
...
pub(crate) fn metadata_has_interactive_execution_contract(metadata: &Value) -> bool {
...
pub(crate) fn message_is_successful_manuscript_write_tool_result(message: &Value) -> bool {
...
pub(crate) fn session_history_has_successful_manuscript_write(
    state: &State<'_, AppState>,
    session_id: &str,
) -> bool {
...
pub(crate) fn clear_stale_completed_interactive_execution_contract(
    state: &State<'_, AppState>,
    session_id: Option<&str>,
) {
    let Some(session_id) = session_id else {
...
pub(crate) fn interactive_execution_contract_instruction(
    contract: &InteractiveExecutionContract,
) -> Option<String> {
...
pub(crate) fn interactive_execution_contract_followup(
    contract: &InteractiveExecutionContract,
    progress: &InteractiveExecutionProgress,
) -> Option<String> {
...
pub(crate) fn interactive_execution_progress_observe_success(
...
pub(crate) fn voice_speech_completed(tool_name: &str, arguments: &Value, result: &Value) -> bool {
...
pub(crate) fn manuscript_write_current_completed(
    tool_name: &str,
    arguments: &Value,
    result: &Value,
) -> bool {
...
#[derive(Debug, Clone)]
pub(crate) struct InteractiveAuthoringSessionTarget {
    pub(crate) project_path: String,
}

pub(crate) fn interactive_authoring_session_target(
    state: &State<'_, AppState>,
    session_id: Option<&str>,
...
pub(crate) fn interactive_authoring_continuation_instruction(
    state: &State<'_, AppState>,
    session_id: Option<&str>,
    tool_name: &str,
    arguments: &Value,
...
pub(crate) fn append_authoring_saved_path_link(
    state: &State<'_, AppState>,
    session_id: Option<&str>,
    content: &str,
) -> String {
...
pub(crate) fn is_authoring_project_link_target(project_path: &str) -> bool {
...
pub(crate) fn authoring_title_from_content(content: &str) -> Option<String> {
...
pub(crate) fn should_replace_authoring_final_content_with_summary(content: &str) -> bool {
    content.chars().count() > 600 || content.trim_start().starts_with('#')
}

pub(crate) fn looks_like_authoring_status_summary(content: &str) -> bool {
...
pub(crate) fn markdown_angle_link_href(href: &str) -> String {
    format!("<{}>", href.trim().replace('>', "%3E"))
}

pub(crate) fn authoring_saved_final_summary(project_path: &str, saved_content: &str) -> String {
    let normalized = normalize_relative_path(project_path);
...
pub(crate) fn emit_loop_guard_checkpoint(
    app: &AppHandle,
    session_id: Option<&str>,
    reason: &str,
...
pub(crate) fn anthropic_tools_for_session(
    state: &State<'_, AppState>,
    runtime_mode: &str,
    session_id: Option<&str>,
) -> Vec<Value> {
...
pub(crate) fn gemini_tools_for_session(
    state: &State<'_, AppState>,
    runtime_mode: &str,
    session_id: Option<&str>,
) -> Vec<Value> {
...
pub(crate) struct GeneratedMediaPreview {
    pub(crate) id: String,
    pub(crate) preview_url: String,
}

pub(crate) fn generated_media_kind_from_tool_result(
    tool_name: &str,
...
pub(crate) fn media_preview_matches_kind(url_or_path: &str, media_kind: &str) -> bool {
...
pub(crate) fn asset_preview_url_from_result(asset: &Value, media_kind: &str) -> Option<String> {
...
pub(crate) fn extract_generated_media_previews_from_tool_result(
    tool_name: &str,
    tool_arguments: &Value,
    result_value: &Value,
...
pub(crate) fn has_generated_media_embed(content: &str, preview_url: &str) -> bool {
    let normalized = content.trim();
...
pub(crate) fn append_generated_media_markdown(
    content: &str,
    heading: &str,
...
pub(crate) fn append_generated_media_sections(
    content: &str,
...
pub(crate) fn interactive_tool_round_fallback_response(
    outcomes: &[InteractiveToolOutcomeDigest],
) -> Option<String> {
...
#[derive(Debug, Clone)]
pub(crate) struct PendingInteractiveToolCall {
    pub(crate) call_id: String,
    pub(crate) tool_name: String,
}

pub(crate) fn interactive_tool_abort_message() -> &'static str {
    "工具调用已中止：本轮运行在返回结果前结束。"
}

pub(crate) fn append_aborted_interactive_tool_result(
    state: &State<'_, AppState>,
    session_id: &str,
    call_id: &str,
    tool_name: &str,
...
pub(crate) struct InteractiveToolCallGuard<'a> {
    pub(crate) app: &'a AppHandle,
    pub(crate) state: &'a State<'a, AppState>,
    pub(crate) session_id: Option<&'a str>,
    pub(crate) pending: Vec<PendingInteractiveToolCall>,
}
...
pub(crate) fn interactive_tool_panic_message(
    tool_name: &str,
...
pub(crate) fn run_anthropic_interactive_chat_runtime(
    app: &AppHandle,
    state: &State<'_, AppState>,
    session_id: Option<&str>,
...
pub(crate) fn run_gemini_interactive_chat_runtime(
    app: &AppHandle,
    state: &State<'_, AppState>,
    session_id: Option<&str>,
    config: &ResolvedChatConfig,
...
pub(crate) fn run_openai_interactive_chat_runtime(
    app: &AppHandle,
    state: &State<'_, AppState>,
    session_id: Option<&str>,
    config: &ResolvedChatConfig,
...
pub(crate) fn parse_youtube_channel(url: &str) -> (String, String) {
...
pub(crate) fn build_advisor_youtube_channel(
...
pub(crate) fn decode_local_path_segment(raw: &str) -> String {
...
pub(crate) fn trim_windows_drive_url_prefix(value: &str) -> &str {
...
pub(crate) fn file_url_to_local_path(parsed: &url::Url) -> Option<PathBuf> {
...
pub(crate) fn subject_data_url_extension(meta: &str, fallback: &str) -> String {
    let mime = meta
        .split(';')
        .next()
        .unwrap_or("")
...
pub(crate) fn subject_input_file_extension(name: Option<&str>, fallback: &str) -> String {
...
pub(crate) fn safe_subject_relative_path(raw: &str) -> Option<String> {
...
pub(crate) fn subject_asset_file_name(
    prefix: &str,
    index: usize,
    name: Option<&str>,
    extension: &str,
...
pub(crate) fn materialize_subject_data_url(
    subject_dir: &Path,
    data_url: &str,
    file_name: &str,
) -> Result<String, String> {
...
pub(crate) fn materialize_subject_image_paths(
    subject_dir: &Path,
...
pub(crate) fn materialize_subject_voice_path(
    subject_dir: &Path,
...
pub(crate) fn materialize_subject_video_path(
    subject_dir: &Path,
...
pub(crate) fn subject_voice_value_has_id(value: &Value) -> bool {
...
pub(crate) fn subject_voice_sample_file_path(value: &Value) -> Option<String> {
...
pub(crate) fn hydrated_subject_record(
    subjects_root: &Path,
    mut record: SubjectRecord,
) -> SubjectRecord {
...
pub(crate) fn subject_catalog_item(record: &SubjectRecord) -> Value {
    json!({
        "id": record.id,
        "name": record.name,
...
pub(crate) fn persist_subjects_workspace(
    subjects_root: &Path,
...
pub(crate) fn build_subject_record_for_workspace(
    subjects_root: &Path,
...
pub(crate) fn subject_category_name<'a>(
...
pub(crate) fn validate_subject_brand_binding(
    input: &SubjectMutationInput,
    categories: &[SubjectCategory],
    subjects: &[SubjectRecord],
) -> Result<(), String> {
...
pub(crate) fn handle_subject_category_create(
    payload: Value,
    state: &State<'_, AppState>,
) -> Result<Value, String> {
...
pub(crate) fn handle_subject_category_update(
    payload: Value,
    state: &State<'_, AppState>,
) -> Result<Value, String> {
    persistence::ensure_store_hydrated_for_subjects(state)?;
...
pub(crate) fn handle_subject_category_delete(
    payload: Value,
    state: &State<'_, AppState>,
) -> Result<Value, String> {
    persistence::ensure_store_hydrated_for_subjects(state)?;
...
pub(crate) fn handle_subject_create(
    payload: Value,
    app: &AppHandle,
    state: &State<'_, AppState>,
) -> Result<Value, String> {
...
pub(crate) fn handle_subject_update(
    payload: Value,
    app: &AppHandle,
    state: &State<'_, AppState>,
) -> Result<Value, String> {
...
pub(crate) fn handle_subject_delete(
    payload: Value,
    state: &State<'_, AppState>,
) -> Result<Value, String> {
    persistence::ensure_store_hydrated_for_subjects(state)?;
        ...
        || channel == "wander:brainstorm"
    {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            if channel_name == "wander:brainstorm" {
                match channel_router::handle_channel(
                    &app_handle,
                    &channel_name,
                    payload_value.clone(),
                    &managed_state,
