...
pub(crate) fn ensure_supported_space(
...
pub(crate) fn imported_docs_root(state: &State<'_, AppState>) -> Result<PathBuf, String> {
...
pub(crate) fn ingest_xhs_entry_v2(
    ...
    Ok(json!({
        ...
        "entryId": entry_id,
        "kind": note_kind,
        "duplicate": note_response.get("duplicate").and_then(|value| value.as_bool()).unwrap_or(false),
        "updated": note_response.get("updated").and_then(|value| value.as_bool()).unwrap_or(false),
        "comments": comments,
        "received": {
...
mod tests {
    use super::{
        ...
        maybe_backfill_xiaohongshu_assets, normalized_xhs_comment_values, note_entry_id,
        note_transcript_file_from_meta, should_auto_transcribe_knowledge_video,
        should_retry_note_transcription_error, xhs_comments_has_payload, xhs_note_has_payload,
        xhs_v2_entry_request, youtube_entry_id, zhihu_answer_to_entry_request,
...
mod source_normalizers;

use source_normalizers::*;
