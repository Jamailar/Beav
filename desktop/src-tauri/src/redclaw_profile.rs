...
fn build_default_agent_profile_doc() -> String {
    ...
    vec![
        ...
        "- 如果当前任务带有已确认的 `team_creation` 弹窗意图，说明用户已授权你根据目标自动规划成员；这次提交就是创建确认，不要再让用户选择成员。".to_string(),
...
fn build_default_space_writing_style_skill_doc() -> String {
    [
        ...
        "contextNote: 当前空间提供一份默认 writing-style 模板，必要时可按需激活。写作前先读取用户档案和创作者档案，理解自媒体创作目标、目标读者、内容边界和长期风格。完成风格初始化后，这份技能会升级为该空间的专属写作规则。",
        "promptPrefix: 当当前任务明确是写作、改写、润色或选题 framing 时，再应用这份默认 writing-style；若用户尚未完成风格初始化，先按这份基础模板写。动笔前先读取 `profiles://user` 与 `profiles://creator_profile`，或调用 `Operate(resource=\"redclaw.profile\", operation=\"bundle\", input={})` 获取档案包，理解用户的自媒体创作目标和长期边界。",
...
fn build_space_writing_style_skill(
    ...
    let mut lines = vec![
        ...
        "autoActivate: true".to_string(),
        "activationScope: turn".to_string(),
        format!(
            "contextNote: 当前空间存在一份可按需激活的 writing-style。写作前先读取用户档案和创作者档案，理解自媒体创作目标、目标读者、内容边界和长期风格。当前空间风格画像为 {}；经营模式 {}；账号角色 {}。",
            summary_headline, primary_model_label, role_position_label
        ),
        format!(
            "promptPrefix: 当当前任务明确涉及写作、改写、润色或选题 framing 时，再应用这份 writing-style。动笔前先读取 `profiles://user` 与 `profiles://creator_profile`，或调用 `Operate(resource=\"redclaw.profile\", operation=\"bundle\", input={{}})` 获取档案包，理解用户的自媒体创作目标和长期边界。写作任务应体现 {}；开头方式 {}；转化表达 {}。",
            summary_headline,
            opening_label,
        ...
        "- 写作前先读取用户档案和创作者档案，理解用户是谁、账号做什么、服务谁、长期创作目标是什么、哪些表达边界不能碰。".to_string(),
        "- 档案读取优先使用 `profiles://user`、`profiles://creator_profile`，或 `Operate(resource=\"redclaw.profile\", operation=\"bundle\", input={})`；不要只凭当前素材猜用户定位。".to_string(),
        "- 档案只用于校准方向、语气、边界和读者，不要把档案内容大段复述进正文。".to_string(),
        ...
        "".to_string(),
        "## 写作前流程".to_string(),
        "1. 先读档案：确认用户画像、自媒体定位、目标读者、商业目标、内容风格和禁区。".to_string(),
        "2. 再读素材：判断素材只是启发、证据、细节，还是可以作为正文核心。".to_string(),
        "3. 先定创作目标：这篇内容要让哪类读者产生什么判断、情绪或行动。".to_string(),
        "4. 再写标题和正文：标题、开头、正文推进和 CTA 都要服务于这个目标，而不是服务于素材覆盖率。".to_string(),
        "".to_string(),
        "## 经营方向".to_string(),
    ];
