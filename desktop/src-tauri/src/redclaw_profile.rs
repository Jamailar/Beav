...
fn build_default_space_writing_style_skill_doc() -> String {
    [
        ...
        "- 禁止编造案例、数据、用户反馈和个人经历。",
        "- 禁止输出分页符、page-break、孤立分隔线或宿主会误判的分页内容。",
        "",
        "## 初始化提示",
...
fn build_space_writing_style_skill(
    ...
    let mut lines = vec![
        ...
        "promptSuffix: 如果当前任务不是写作，不要让 writing-style 主导其他决策；如果当前任务是写作，标题、开头、正文、CTA 都必须体现当前空间设定。标题控制在 20 个汉字以内；禁止模板化 AI 套话、编造经历、分页符和孤立分隔线。".to_string(),
        ...
        "- 禁止输出分页符、page-break、孤立分隔线和会触发宿主分页解析的分隔内容。".to_string(),
        "".to_string(),
        "## 经营方向".to_string(),
    ];
