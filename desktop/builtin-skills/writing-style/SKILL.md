...
promptSuffix: 如果当前任务不是写作，就不要让 writing-style 主导其他决策；如果当前任务是写作，标题、内容方向、正文和文案细节都必须体现这份技能的约束。所有标题必须控制在 20 个汉字以内，或其它语言下等价的简洁长度。写稿时禁止输出分页符、分页标记或会触发宿主分页解析的分隔线。
maxPromptChars: 2400
---
# Writing Style
...
- 写正文时禁止输出分页符、`page-break`、孤立分隔线（如单独一行的 `---` / `***` / `___`）、`\f`，也不要故意插入连续三个空行；正文只输出正常内容结构。
