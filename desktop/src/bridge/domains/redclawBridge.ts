...
export function createRedClawBridge(core: BridgeCore) {
  return {
    redclawRunner: {
      ...
      setScheduledEnabled: (payload: { taskId: string; enabled: boolean }) => core.invokeChannel('redclaw:runner-set-scheduled-enabled', payload),
      ...
      setLongCycleEnabled: (payload: { taskId: string; enabled: boolean }) => core.invokeChannel('redclaw:runner-set-long-cycle-enabled', payload),
    ...
    redclawOrchestration: {
      createRun: (payload: { goal: string; sessionId?: string; projectId?: string; platform?: string; format?: string }) =>
        core.invokeChannel('redclaw:orchestration-create-run', payload),
      getRegistry: () => core.invokeChannel('redclaw:orchestration-registry'),
    },
    redclawProjects: {
      ...
      updateLearningCandidate: (payload: { projectId: string; candidateId: string; status: 'accepted' | 'rejected' | 'pending' }) =>
        core.invokeChannel('redclaw:learning-candidate-update', payload),
      updateSection: (payload: { projectId: string; sectionId: string; content: string }) =>
        core.invokeChannel('redclaw:project-section-update', payload),
      exportMediaPlan: (payload: { projectId: string }) =>
        core.invokeChannel('redclaw:media-plan-export', payload),
      renderRoughCut: (payload: { projectId: string }) =>
        core.invokeChannel('redclaw:media-plan-render', payload),
      exportPublishPackage: (payload: { projectId: string }) =>
        core.invokeChannel('redclaw:publish-package-export', payload),
      exportReviewReport: (payload: { projectId: string }) =>
        core.invokeChannel('redclaw:review-report-export', payload),
      exportXhsPackage: (payload: { projectId: string }) =>
        core.invokeChannel('redclaw:xhs-package-export', payload),
    },
    redclawProfile: {
      ...
      startStyleDefinition: (payload?: { forceRestart?: boolean; source?: string; sessionId?: string }) =>
        core.invokeChannel('redclaw:profile:start-style-definition', payload || {}),
      completeStyleDefinition: (payload: Record<string, unknown>) =>
        core.invokeChannel('redclaw:profile:complete-style-definition', payload),
    },
  };
}
