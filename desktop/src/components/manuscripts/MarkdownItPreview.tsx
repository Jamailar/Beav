...
export function MarkdownItPreview({
  content,
  className,
  density = 'default',
  emptyText = '开始写内容后，这里会显示正文。',
}: MarkdownItPreviewProps) {
  const html = useMemo(
    () => {
      const { body } = parseMarkdownFrontmatter(content);
      return markdownRenderer.render(body.trim() || emptyText);
    },
    [content, emptyText]
  );
  ...
  return (
    <div
      className={clsx(
        ...
        compact
          ? '[&_h1]:text-[1.65rem] [&_h1]:font-semibold [&_h1]:leading-tight [&_h1]:tracking-tight [&_h1]:text-text-primary'
          : '[&_h1]:text-[2rem] [&_h1]:font-semibold [&_h1]:leading-tight [&_h1]:tracking-tight [&_h1]:text-text-primary',
        compact
          ? '[&_h2]:mt-8 [&_h2]:text-[1.25rem] [&_h2]:font-semibold [&_h2]:leading-tight [&_h2]:tracking-tight [&_h2]:text-text-primary'
          : '[&_h2]:mt-10 [&_h2]:text-[1.45rem] [&_h2]:font-semibold [&_h2]:leading-tight [&_h2]:tracking-tight [&_h2]:text-text-primary',
        compact
          ? '[&_h3]:mt-6 [&_h3]:text-[1.05rem] [&_h3]:font-semibold [&_h3]:leading-tight [&_h3]:text-text-primary'
          : '[&_h3]:mt-8 [&_h3]:text-[1.15rem] [&_h3]:font-semibold [&_h3]:leading-tight [&_h3]:text-text-primary',
        compact
          ? '[&_h4]:mt-5 [&_h4]:text-base [&_h4]:font-semibold [&_h4]:text-text-primary'
          : '[&_h4]:mt-6 [&_h4]:text-lg [&_h4]:font-semibold [&_h4]:text-text-primary',
        compact
          ? '[&_p]:my-0 [&_p]:text-[15px] [&_p]:leading-7 [&_p]:text-text-secondary'
        ...
        compact
          ? '[&_pre]:my-5 [&_pre]:overflow-x-auto [&_pre]:rounded-xl [&_pre]:border [&_pre]:border-border [&_pre]:bg-surface-secondary [&_pre]:p-4'
        ...
        compact
          ? '[&_td]:border-b [&_td]:border-border [&_td]:px-3 [&_td]:py-2 [&_td]:text-[13px] [&_td]:leading-6 [&_td]:text-text-secondary'
