...
export function WanderLoadingDice({ className, size = 92 }: WanderLoadingDiceProps) {
  ...
  return (
    <div
      aria-hidden="true"
      className={clsx('wander-loading-dice relative flex items-center justify-center', className)}
      style={frameStyle}
    >
      <div
        className="wander-loading-dice__halo absolute rounded-full blur-[30px]"
        style={{ width: haloSize, height: haloSize }}
      />
      <div
        className="wander-loading-dice__shadow absolute bottom-2 rounded-full blur-md"
      ...
      <div className="wander-loading-dice__float relative flex items-center justify-center [transform-style:preserve-3d]">
        ...
        >
          {FACE_LAYOUTS.map(({ value, transform }) => (
            <div
              key={value}
              className="wander-loading-dice__face absolute inset-0 overflow-hidden rounded-[26px] border"
              style={{ transform: transform(depth) }}
            >
              <div className="wander-loading-dice__face-sheen absolute inset-[1px] rounded-[24px]" />
              <div className="wander-loading-dice__face-highlight absolute inset-x-3 top-2 h-4 rounded-full blur-sm" />
              {FACE_PIPS[value].map(([left, top], index) => (
                <span
                  key={`${value}-${index}`}
                  className="wander-loading-dice__pip absolute rounded-full"
                  style={{
                    width: pipSize,
                    height: pipSize,
                    left: `${left}%`,
