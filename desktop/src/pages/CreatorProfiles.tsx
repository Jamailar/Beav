                                ...
                                {loadingDetail ? (
                                    ...
                                    <div className="space-y-5">
                                        <div className="grid gap-2 sm:grid-cols-3">
                                            <Metric label="内容" value={selectedAccount.postCount || posts.length} compact />
                                            <Metric label="媒体" value={selectedAccount.mediaCount || media.length} compact />
                                            <Metric label="评论" value={selectedAccount.commentCount || comments.length} compact />
                                        </div>
                                        ...
                                        <section>
                                            <div className="mb-3 text-sm font-medium text-text-primary">最近内容</div>
                                            <PostGallery posts={posts.slice(0, 4)} compact />
                                        </section>
                                    </div>
                                )}
                            </div>
                ...
                <div className={clsx('flex-1 min-h-0 overflow-y-auto space-y-2', compact ? 'p-2' : 'p-3')}>
                    {loadingAccounts && accounts.length === 0 ? (
                        ...
                        <div className={clsx('rounded-lg border border-dashed border-border bg-surface-primary text-sm text-text-secondary', compact ? 'p-3' : 'p-4')}>
                            {compact ? '暂无账号档案' : '当前空间还没有绑定账号。打开插件 sidepanel，在账号主页绑定后，这里会显示账号画廊和学习结果。'}
                        </div>
                    ) : accounts.map((account) => (
                        <button
                            key={account.id}
            ...
            <main className="flex-1 min-w-0 flex flex-col">
                <header className={clsx('border-b border-border bg-surface-primary', compact ? 'px-4 py-3' : 'px-6 py-4')}>
                    <div className="flex items-start justify-between gap-4">
                        ...
                        <div className={clsx('grid grid-cols-3 shrink-0', compact ? 'gap-1.5' : 'gap-2')}>
                            <Metric label="内容" value={selectedAccount?.postCount || posts.length} compact={compact} />
                            <Metric label="媒体" value={selectedAccount?.mediaCount || media.length} compact={compact} />
                            <Metric label="评论" value={selectedAccount?.commentCount || comments.length} compact={compact} />
                        </div>
                    </div>
                </header>
                    ...
                    {!selectedAccount ? (
                        ...
                        <div className={clsx(compact ? 'p-4 space-y-4' : 'p-6 space-y-6')}>
                            ...
                            <section className={clsx(
                                'grid grid-cols-1',
                                compact ? 'xl:grid-cols-[minmax(0,1fr)_280px] gap-4' : 'xl:grid-cols-[minmax(0,1fr)_360px] gap-6',
                            )}>
                                <div className="min-w-0 space-y-4">
                                    ...
                                    {loadingDetail ? (
                                        ...
                                        <PostGallery posts={filteredPosts} compact={compact} />
                                    ) : galleryMode === 'media' ? (
                                ...
                                <aside className={clsx(compact ? 'space-y-3' : 'space-y-4')}>
                                    <InsightPanel
                                        title="创作档案"
                                        icon={<Bot className="w-4 h-4" />}
                                        content={detail?.creatorProfile || detail?.learningSummary || ''}
                                        empty={compact ? '暂无档案' : '还没有生成创作档案。导入账号内容后会自动学习。'}
                                        compact={compact}
                                    />
                                    <InsightPanel
                                        title="写作风格"
                                        ...
                                        content={detail?.writingStyleSkill || ''}
                                        empty={compact ? '暂无风格' : '还没有写作风格技能。'}
                                        compact={compact}
                                    />
                                    {!compact && <section className="rounded-lg border border-border bg-surface-secondary/30 p-4">
    ...
    return (
        ...
        >
            ...
            <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                <div>
                    <div className="text-sm font-semibold tabular-nums text-text-primary">{numberText(account.postCount)}</div>
                    <div className="mt-0.5 text-[11px] text-text-tertiary">内容</div>
                </div>
                <div>
                    <div className="text-sm font-semibold tabular-nums text-text-primary">{numberText(account.mediaCount)}</div>
                    <div className="mt-0.5 text-[11px] text-text-tertiary">媒体</div>
                </div>
                <div>
                    <div className="text-sm font-semibold tabular-nums text-text-primary">{numberText(account.commentCount)}</div>
                    <div className="mt-0.5 text-[11px] text-text-tertiary">评论</div>
                </div>
            </div>
...
function PostGallery({ posts, compact = false }: { posts: AccountPost[]; compact?: boolean }) {
    if (posts.length === 0) {
        return <EmptyGallery text="没有匹配的内容" />;
    }
    return (
        <div className={clsx('grid grid-cols-1 md:grid-cols-2', compact ? '2xl:grid-cols-2 gap-3' : '2xl:grid-cols-3 gap-4')}>
            {posts.map((post) => {
                const cover = postCover(post);
                return (
                    <article key={post.id} className="rounded-lg border border-border bg-surface-secondary/30 overflow-hidden">
                        <div className={clsx('bg-surface-secondary flex items-center justify-center', compact ? 'aspect-[16/9]' : 'aspect-[4/3]')}>
                            {cover ? (
                        ...
                        <div className={clsx(compact ? 'p-3' : 'p-4')}>
                            ...
                            {!compact && <div className="mt-2 text-xs text-text-secondary line-clamp-3">{excerpt(post.content, 150) || '无正文摘要'}</div>}
...
function InsightPanel({ title, icon, content, empty, compact = false }: { title: string; icon: ReactNode; content: string; empty: string; compact?: boolean }) {
    const text = content.trim();
    return (
        <section className={clsx('rounded-lg border border-border bg-surface-secondary/30', compact ? 'p-3' : 'p-4')}>
            <div className="flex items-center gap-2 text-sm font-medium text-text-primary">
                {icon}
                {title}
            </div>
            <div className={clsx('mt-3 overflow-y-auto rounded-md bg-surface-primary border border-border p-3 text-xs leading-5 text-text-secondary whitespace-pre-wrap', compact ? 'max-h-28' : 'max-h-72')}>
                {text ? excerpt(text, compact ? 360 : 1800) : empty}
            </div>
        </section>
    );
}
