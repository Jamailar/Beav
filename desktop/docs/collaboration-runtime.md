...
Collaboration Runtime 是 Team / ACP / Workboard 的宿主侧控制面。它保存结构化协作状态，并通过受控 runner 拉起外部 ACP/CLI 成员；成员执行器、ACP 适配器和 UI 看板都必须通过这里读写协作状态。
...
- `team-runtime:list-agent-backends`: 返回可用内部 runtime 和本机动态发现到的可执行 agent CLI；不返回硬编码 missing 项。
...
- `team-runtime:mcp-bridge-config`: 生成外部 ACP 成员可注入的 `redbox-team` stdio bridge 配置。
- `team-runtime:execute-mcp-tool`: 在宿主内执行一个 `redbox-team` MCP 工具调用。
- `team-runtime:run-external-member`: 拉起外部 ACP/CLI 成员执行任务，并把退出结果写回 member/task/report。
...
当前层是 host-owned state machine + controlled external runner。成员执行器接入时必须遵守：

- 内部成员执行器只能通过 `team-runtime:submit-report` 和 `team-runtime:update-task` 汇报状态。
- ACP 适配器必须把外部 client/server 状态映射成 `CollabMemberRecord`、`CollabTaskRecord` 和 `CollabProgressReportRecord`。
- 真实工具执行仍走现有 Runtime / tools / approval 边界，不允许在 collab runtime 内调用 agent，避免形成嵌套黑盒。
- `run-external-member` 只负责受控进程生命周期、MCP 环境注入、stdout/stderr 捕获和状态回写；它不解析自由文本协议做隐藏路由。
- 外部进程会获得 `REDBOX_TEAM_SESSION_ID`、`REDBOX_TEAM_MEMBER_ID`、`REDBOX_TEAM_TASK_ID` 和 `REDBOX_TEAM_MCP_CONFIG`，用于接入 `redbox-team` MCP 工具。
