...
- Added MCP search results to `tool_search`.
...
- 2026-05-04 update: MCP deferred discovery now flows through first-class `tool_search`.
- 2026-05-05 update: removed the old `tools.search` app_cli compatibility action; deferred discovery has a single model-visible entry.
...
RedConvert 已有 `tool_search` 和 deferred action index，可以把同一策略扩展到 MCP：
...
- 若没有 deferred MCP 或 deferred RedBox action，不注册 `tool_search`。
...
- `tool_search` 的索引合并 RedBox deferred actions 和 MCP deferred tools，但结果类型要区分 `redbox_action` / `mcp_tool`。
...
- 未 direct 但在 deferred 里的 MCP tool，返回结构化 `TOOL_DEFERRED`，提示用 `tool_search`。
...
- 大工具集提示模型先用 `tool_search`，不要手写 server method。
...
- 合并 MCP deferred search entries 到 `tool_search`。
...
- `tool_search` 只在存在 deferred 来源时可见。
...
| Large tool set | direct 工具受预算限制，其余进入 `tool_search` |
...
- `tool_search` 能发现 deferred MCP tools，并且搜索后可调用。
