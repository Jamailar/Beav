...
- team collaboration：
  ...
  - 打开 Workboard 的 Collaboration 模式，验证 session 切换、member roster、Kanban、report request、内部成员创建和 stale-while-revalidate
  - 在主 AI 对话中要求“创建一个协作项目，包含 2 个内部成员和任务看板”，验证 AI 使用 `app_cli(action="team.session.create"|"team.member.spawn"|"team.task.create")`
