...
- Extracted media transcription estimated SRT/VTT subtitle rendering, cue splitting and provider-error rejection into `commands/media_transcribe/subtitles`, keeping transcription request routing and fallback order unchanged.
...
- Extracted manuscript FFmpeg edit package-asset extraction, asset path resolution and operation input fallback helpers into `commands/manuscripts/ffmpeg_edit/assets`, keeping recipe execution and CLI verification in `ffmpeg_edit.rs`.
- Extracted manuscript FFmpeg edit seconds formatting, output-path allocation and managed CLI ffmpeg execution wrapper into `commands/manuscripts/ffmpeg_edit/execution`, leaving `ffmpeg_edit.rs` focused on recipe operation sequencing.
...
- Extracted manuscript content-block Markdown/HTML fragment rendering into `commands/manuscripts/content_blocks/rendering`, keeping public render helpers re-exported through `content_blocks`.
- Extracted manuscript content-block Markdown heading/paragraph/page-break parsing and block match-key normalization into `commands/manuscripts/content_blocks/parsing`, keeping stable-id matching and content-map projection in `content_blocks.rs`.
...
- Extracted manuscript editor project motion track, animation-layer access, motion item normalization and Remotion motion sync helpers into `commands/manuscripts/editor_project_model/motion`, keeping the existing project-model re-export surface intact.
...
- Extracted manuscript richpost render-model CSS token layering, style-attribute serialization and token lookup into `commands/manuscripts/richpost_render_model/style`, keeping page HTML and preview shell assembly in the root render module.
...
- Extracted manuscript richpost page artifact persistence for page HTML writes, stale page cleanup and preview shell writes into `commands/manuscripts/richpost_artifacts.rs`, leaving root command code focused on page plan normalization and package state refresh.
- Extracted manuscript richpost page plan generation and normalization helpers into `commands/manuscripts/richpost_plan.rs`, keeping root command code focused on persistence orchestration and package state refresh.
- Extracted manuscript richpost template normalization and template-to-master role/name rules into `commands/manuscripts/richpost_plan/templates`, leaving page-plan generation focused on block, asset and zone assembly.
- Extracted manuscript richpost zone assignment, zone id projection, fallback zone construction and zone/style normalization into `commands/manuscripts/richpost_zones.rs`.
- Extracted manuscript package/editor script approval state, video brief persistence, video asset-kind normalization and disabled animation-director stub into `commands/manuscripts/script_state.rs`, keeping sibling command modules on shared script-state helpers.
- Extracted manuscript write-proposal lookup, upsert, reject and accept helpers into `commands/manuscripts/write_proposals.rs`, preserving the existing tree command API while isolating proposal store mutation and event emission.
...
- Extracted manuscript video package project-state projection and video brief persistence channel handlers into `commands/manuscripts/package_video.rs`, leaving `package.rs` focused on package routing, script state, transcription and import flows.
...
- Split manuscript editor AI command planning, motion-item generation and AI command normalization into `commands/manuscripts/editor_ai_commands.rs`, keeping `editor_commands.rs` focused on applying validated commands to project state.
...
- Extracted official billing product endpoint fallback querying and fallback-product projection into `commands/official/billing/products`, keeping product channel handlers focused on endpoint order and settings update.
...
- Extracted skills AI role listing, protocol detection and connection self-check responses into `commands/skills_ai/ai_control`, leaving `skills_ai.rs` focused on skill CRUD, invocation and installation dispatch.
...
- Extracted media edit ffmpeg seconds formatting, output path allocation and managed CLI execution wrapper into `commands/media_edit/execution`, leaving `pipeline.rs` focused on operation sequencing and artifact projection.
- Extracted manuscript editor command string-id parsing, object-patch merging, default track UI, next-track id calculation and track order renumbering into `commands/manuscripts/editor_commands/actions`, leaving `editor_commands.rs` focused on applying editor commands in protocol order.
...
- Extracted advisor YouTube channel id/name/description/avatar projection from yt-dlp payloads into `commands/advisor_ops/youtube_channel_info`, keeping `youtube.rs` focused on orchestration and persistence calls.
- Extracted advisor YouTube single-video subtitle download and failed-video retry handling into `commands/advisor_ops/youtube_video_download`, moving retry subtitle downloads outside the mutable store lock and keeping lock scope to snapshot/writeback.
- Extracted RedClaw runner scheduled/long-cycle UI payload to `TaskIntentSchema` mapping into `commands/redclaw/runner_tasks/payloads`, leaving `runner_tasks.rs` focused on channel handling, store mutation, manual enqueue and scheduler emission.
- Extracted file preview extension, display-name, preview-kind and MIME inference into `commands/file_ops/preview_types`, leaving `preview.rs` focused on path resolution, package fallback and preview response assembly.
- Extracted file preview virtual path sanitization, virtual resource root resolution, manuscript package fallback and redbox asset URL construction into `commands/file_ops/preview_paths`, keeping preview response assembly separate from path security policy.
- Extracted Xiaohongshu V2 knowledge source ingest request contracts, note-to-entry mapping, comment normalization, comments artifact persistence and focused tests into `knowledge/xhs`, leaving `knowledge.rs` as the generic knowledge entry/batch/http owner.
- Extracted richpost zone rendering, asset/fragment/block ordering and master-fragment zone replacement into `commands/manuscripts/richpost_render_model/zones`, leaving `richpost_render_model.rs` focused on CSS token projection, page HTML and preview shell assembly.
- Extracted richpost preview shell HTML, iframe query propagation and shell token projection into `commands/manuscripts/richpost_render_model/preview_shell`, leaving `richpost_render_model.rs` focused on page HTML and master fragment orchestration.
- Extracted legacy richpost theme directory/index migration, background asset relocation and old-store cleanup into `commands/manuscripts/theme/store/migration`, leaving `theme/store.rs` focused on current theme store reads, writes, catalog selection and path helpers.
- Extracted richpost pagination typography metrics, wrapped text estimation, block-height calculation and paragraph splitting into `commands/manuscripts/richpost_pagination/metrics`, leaving `richpost_pagination.rs` focused on page draft assembly and pagination strategy.
- Extracted manuscript package subtitle transcription settings resolution, media-source validation, SRT clipping and subtitle timeline writeback into `commands/manuscripts/package_subtitles`, leaving `package.rs` focused on package channel dispatch, script confirmation and file attachment.
- Extracted editor project subtitle default style construction and last-transcription AI metadata writeback into `commands/manuscripts/editor_project_model/subtitles`, leaving `editor_project_model.rs` focused on tracks, motion items and timeline normalization.
...
- Extracted MCP server registry list/save/add/get/remove/enable/import/OAuth-status handling into `commands/mcp_tools/registry`, keeping `mcp_tools.rs` focused on diagnostics, invocation, typed lists, hooks and channel dispatch.
- Extracted MCP runtime hook list/register/remove channel handling into `commands/mcp_tools/runtime_hooks`, keeping `mcp_tools.rs` focused on MCP diagnostics, invocation, typed lists and channel dispatch.
...
- Extracted RedClaw learning-candidate and project-section update mutations into `commands/redclaw/project_updates`, leaving `redclaw.rs` focused on dispatch and orchestration team creation.
- Moved official command tests into `commands/official/tests.rs`, leaving `official.rs` as the public channel router plus shared official helper facade.
...
- Extracted collaboration team-action result event fanout and mailbox value parsing into `commands/runtime_collab/team_events`, leaving `team_wake` focused on wake preparation, execution, completion and rescheduling.
...
- Extracted advisor create, update and delete channel handling, member-skill refresh hooks and advisor-change event emission into `commands/advisor_ops/crud`.
...
- Extracted collaboration session creation/listing/status/coordinator handling and snapshot assembly into `runtime/collab_runtime/session_core`.
