...
2. 自定义 LLM 源：在登录页输入 API Base URL 和 API Key，自动检测协议，由 `ai_model_manager` 设置默认聊天模型，保存为默认 AI 源，然后进入 App。
...
- `desktop/src/pages/Settings.tsx` 已有 AI 源管理、默认模型保存、route 配置保存。
...
- `desktop/src-tauri/src/commands/skills_ai.rs` 已有 `ai:detect-protocol`、`ai:test-connection`。
- `desktop/src-tauri/src/ai_model_manager` 是模型配置、route 和 readiness 的唯一治理出口。
...
1. 用户传入的 `preferredModel`。
2. `ai_model_manager` 按 provider/protocol 给出的默认聊天模型。
3. 如果 route 缺少模型，则 readiness false，不保存为 ready 状态。

登录页默认路径应一键完成；高级模型选择进入 Settings，由统一 manager 治理。
...
自定义 API tab:
  ...
  API Key
  Continue
  inline status: 正在保存模型配置 / 已选择 xxx / 错误原因
```
...
- 模型配置治理：复用 `ai_model_manager`，不再通过通用远端枚举选择模型。
...
- 有自定义源但模型为空：readiness false，登录页显示自定义 API tab，并允许重新保存模型配置。
...
- 复用 `infer_protocol`，模型默认值交给 `ai_model_manager` 投影和 route 解析。
- 保存 `ai_sources_json/default_ai_source_id/api_endpoint/api_key/model_name/ai_model_routes_json`。
