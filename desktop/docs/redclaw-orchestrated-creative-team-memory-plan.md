...
RedClaw 默认先作为直接执行者响应用户。只有任务明显需要多个专业岗位接力时，RedClaw 才进入团队编排模式。团队不是用户手动创建的 UI 对象，而是 RedClaw 根据目标临时激活、分配任务、执行后释放的运行结构。
...
- 明确要求全流程、端到端、一站式、团队协作或多 Agent 协作。
- 需要研究资料、找选题、写稿、配图/视频、发布包、质检、复盘等多个步骤接力。
- 需要从知识库、收藏、外部链接、本地素材中提取证据再创作。
- 同时要求三个及以上交付物，例如标题、封面、正文、标签、配图、发布建议。
- 需要合规复核、风格学习、表现复盘或长期记忆沉淀。

实现上必须有两层保护：RedClaw 提示词先要求模型判断工作形态；`redclaw_orchestration` 后端规划器再拒绝 trivial/simple goal，避免普通聊天被误拉进团队 run。
