...
- Agent-side JS client：`scripts/browser-client.mjs` 提供 Codex 同款 `setupBrowserRuntime({ globals }) -> agent.browsers.get("extension")` 入口，用对象方法包装 `browser.user.openTabs()`、`browser.tabs.new()`、`tab.playwright.locator()`、`browser.tabs.finalize()` 等能力；底层仍走 RedBox native-host socket / MCP 工具。
...
不传 `--extension-id` 时，安装器会尝试从 Chrome / Edge / Brave 等 profile 里发现已加载的 RedBox unpacked extension。`--node` 默认使用当前 Node；面向真实 Google Chrome 时建议传绝对 Node 22 路径，避免 GUI Chrome 的 PATH 与终端不同。
...
- “打开网页读取内容”不是浏览器控制验收；必须看到 RedBox MCP / native host socket 经真实 Chrome 扩展返回 `tools/list`、`tabs.list`、`tab.info`、DOM 查询和至少一个交互动作。
- `pnpm smoke:browser-control` 使用临时 profile / Chromium 做回归，不代表用户真实 Chrome 可用。
- 真实 Chrome 验收必须使用已安装的 RedBox Capture 扩展、真实 Chrome native messaging manifest、真实 socket，以及真实标签页或受控测试标签页。
- 被 `tab.claim` / `tab.create` 纳入 active browser session 的页面必须显示 `RedBox 控制中` 页面内标签；释放、finalize 或 turn 结束后自动移除。
...
- `scripts/browser-client.mjs`：供 agent / 调试脚本按 Codex Browser Use 对象 API 使用 RedBox browser-control；配套文档在 [Plugin/docs/browser-runtime.md](/Users/Jam/LocalDev/GitHub/RedConvert/Plugin/docs/browser-runtime.md)。
- `pnpm install:native-host`：安装 Chrome native messaging host manifest。
- `pnpm diagnose:browser-control`：检查 native host manifest、launcher、RedBox endpoint socket 和 extension forwarding 状态；需要只取报告时加 `-- --no-fail`。
...
- 在已注入页面右侧可使用 RedBox 浮动采集面板；小红书笔记页、YouTube、抖音、公众号和普通网页会显示不同动作。
