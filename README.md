...
<p align="center">
  <strong>教程：</strong>
  <a href="https://www.bilibili.com/video/BV12LNn6nEem/">Bilibili 视频教程</a>
</p>
...
**Beav（原 RedBox）** 是一个本地优先的 AI 自媒体创作资产工作台。

它解决的问题是：创作者每天会看到大量有价值的内容、评论、图片、视频和选题灵感，但这些素材通常散落在浏览器、收藏夹、截图、聊天记录和文件夹里，无法长期沉淀，也很难被 AI 稳定复用。

### 典型使用收益（基于创作者实际使用场景）

在持续使用 Beav 进行内容生产的过程中，用户通常可以获得以下效率提升（具体取决于内容类型与使用深度）：

* **素材整理时间减少约 2–3 小时 / 天**：浏览器收藏、截图、分散笔记统一进入结构化素材库后显著减少重复整理
* **选题效率提升约 3–5 倍**：基于评论区、历史内容与素材关联检索，减少“从零找灵感”的时间成本
* **内容生产周期显著缩短（约 50%–90% 区间）**：在选题→脚本→素材→成稿的链路中，重复劳动被自动化替代

### 核心能力

Beav 把这些内容收进一个本地工作空间：
你可以通过浏览器插件保存小红书笔记、评论区、YouTube 视频、公众号文章、网页、图片和选中文字，也可以导入本地文档、图片、视频和音频。

这些素材会进入知识库、素材库、媒体库、主体库和稿件工作台，成为后续 AI 创作可以检索、引用和复用的资产。

基于这些资产，Beav 可以帮助你完成：

* 从评论区和历史素材中发现选题机会
* 生成选题 brief、图文稿、视频脚本和口播稿
* 生成封面图、配图、商品图、视频片段和音频素材
* 管理人物、商品、品牌、场景和角色声音等长期主体资产
* 把最终稿件、封面、媒体结果和复盘继续沉淀回工作区

Beav 也可以作为 Codex、Claude Code、Hermes、OpenClaw 等外部 AI Agent 的本地创作资源层。
外部 Agent 负责规划、推理和拆解任务，Beav 负责保存真实素材、提供上下文、执行媒体任务并管理创作产物。
...
当前优先接入方式是本地 **ACP Agent Gateway**：外部 Agent 先通过本机 discovery 文件或 helper 发现当前端口，实时检查 health，再读取 Beav Creator Agent 的 manifest / OpenAPI / guide，创建或复用 ACP 会话，把创作任务以幂等 run 交给 Beav AI，并通过事件游标拿到状态和素材/稿件产物引用。详细方案见 [`desktop/docs/redbox-acp-agent-gateway-implementation-plan.md`](desktop/docs/redbox-acp-agent-gateway-implementation-plan.md)，使用说明见 [`desktop/docs/redbox-acp-agent-gateway-usage.md`](desktop/docs/redbox-acp-agent-gateway-usage.md)，命令行 helper 见 [`desktop/scripts/redbox-acp-client.mjs`](desktop/scripts/redbox-acp-client.mjs)。

外部 Agent 的推荐发现顺序是：读取 `REDBOX_ACP_DISCOVERY_FILE` 或系统默认的 `RedBox/acp-gateway.json`，把文件当作端点提示并访问 `healthUrl` 验证 `ready=true`，再读取 `manifestUrl` / `openapiUrl` / `guideUrl`；如果文件不存在或已失效，再回退到默认 `http://127.0.0.1:31937/acp/v1`。当前线协议明确为自定义 `redbox.acp.v1` HTTP/JSON 方言，并不冒充 A2A 或编辑器 Agent Client Protocol 的线协议兼容实现。
...
Beav 2.5.1 正式版重点修复自定义 API / 本地模型已配置时仍被官方账号登录页拦住的问题。自定义或本地 LLM 已就绪时，用户可以直接进入工作区；同时优化 RedClaw / Chat 的 Agent 工作流，让附件、知识库引用、素材引用和媒体生成请求更稳定地进入任务上下文。
